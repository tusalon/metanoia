...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '02863d44-1b2d-438a-b67f-67b432f30101'; // ID de Metanoia
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '63717298';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'medinasally81@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Metanoia!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'metanoia';
};
...
console.log('✅ config-negocio.js listo para Metanoia');
